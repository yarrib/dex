...
dex context sync --all       # include release/CI/bump churn (off by default)
```

By default the engine keeps **code changes and major decisions only**, dropping
the `[Dependency]` class (releases, version bumps, CI tweaks, docs plumbing).
Design docs are kept (they classify as `[Decision]`). Pass `--all` for everything.
...
- **Keep bodies clean.** Node text is project *history*, not tooling
  boilerplate. The engine strips AI-assistant attribution from commit bodies —
  `Generated by/with [Claude Code](…)` footers, bare `https://claude.ai/code/…`
  links, and `Co-authored-by:` trailers crediting an AI. When you hand-edit a
  node, do the same; preserve genuine human co-authors and issue/PR references.
