...
dex context sync --all      # include [Dependency] churn (off by default)
```

By default the engine keeps **code changes and major decisions only** — it drops
the `[Dependency]` class (releases, version bumps, CI tweaks, docs plumbing) so
the graph stays signal, not churn. Design docs survive (they classify as
`[Decision]`). Use `--all` when you specifically want the mechanical history.
...
Node bodies are project *history*, not tooling boilerplate. The Rust engine
already strips AI-assistant attribution out of commit bodies when it renders a
node — `Generated by/with [Claude Code](…)` footers, bare
`https://claude.ai/code/…` (and `claude.com/claude-code`) links, and
`Co-authored-by:` trailers crediting Claude/Anthropic. When you hand-write or
hand-edit a node, do the same: omit that boilerplate (including any `🤖 Generated
with …` line) and keep only the substance of the change. Preserve genuine human
co-authors and issue/PR references — those are real history.
